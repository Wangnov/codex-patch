...
- The replay launcher runs `codex exec` with repo-local `HOME` under `.codex-runtime/home`, `CODEX_HOME` at `.codex`, and `CODEX_SQLITE_HOME` under `.codex-runtime/sqlite-home`.
- This isolation is intentionally narrow: it blocks the user's global `~/.codex/config.toml` and global `~/.agents/skills`, while still allowing normal environment variable inheritance.
- Repo-local `.codex/config.toml` and `.codex/skills/` are the intended Codex inputs for this repository.
- Repo-local private provider settings must come from environment variables or other non-committed inputs such as `.codex-runtime/codex-exec.env`; never commit a live provider base URL or token into this public repository.
